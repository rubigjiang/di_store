...
/*
#cgo LDFLAGS: -L${SRCDIR} -lplasma-c -lstdc++ -pthread
#include <stdbool.h>
#include "cclient.h"
#include <stdlib.h>
*/
...
const (
	OBJECT_ID_LENGTH          = 20
	MEM_COPY_BLOCK_SIZE       = 1024 * 8
	MEM_COPY_NUM_THREADS      = 8
	MEM_COPY_THRESHOLD_BYTES  = 0 // 0 for not set
	MEM_COPY_BLOCK_SIZE_BYTES = 0 // 0 for not set
	PLASMA_EVICT_IF_FULL      = false
)

var once sync.Once

func ParallelMemCopy(dst unsafe.Pointer, src unsafe.Pointer, nbytes, blockSize, numThreads int) {
	C.ParallelMemCopy(dst, src, C.long(nbytes), C.ulong(blockSize), C.int(numThreads))
}

func ParallelMemCopy2(dst unsafe.Pointer, src unsafe.Pointer, nbytes, blockSize, numThreads int) {
	C.ParallelMemCopy2(dst, src, C.long(nbytes), C.ulong(blockSize), C.int(numThreads))
}
...
func (c *PlasmaClient) Put(ctx context.Context, oid []byte, data []byte) ([]byte, error) {
	...
	msg := C.CreateAndSeal(c.cclient, (*C.char)(poid), (*C.char)(pdata), C.int(len(data)),
		C.bool(PLASMA_EVICT_IF_FULL), C.int(MEM_COPY_NUM_THREADS),
		C.long(MEM_COPY_THRESHOLD_BYTES), C.long(MEM_COPY_BLOCK_SIZE_BYTES))
...
func (c *PlasmaClient) Get(ctx context.Context, oids ...[]byte) ([][]byte, error) {
	...
	for i := 0; i < len(oids); i++ {
		...
		if pData != C.NULL {
			size := int(item.size)
			if size >= MEM_COPY_THRESHOLD_BYTES {
